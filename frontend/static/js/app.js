    ...
    async function fetchStorageInfo() {
        ...
        try {
            ...
            const usedMb = Math.round((data.total_gb - data.available_gb) * 10) / 10;
        const percent = Math.min(100, Math.max(0, (usedMb / data.total_gb) * 100));

        usedEl.textContent = usedMb + " GB";
        totalEl.textContent = data.total_gb;
        freeEl.textContent = Math.round(data.available_gb * 10) / 10 + " GB";
